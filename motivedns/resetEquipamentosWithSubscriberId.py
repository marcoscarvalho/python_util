...
df = pd.read_csv('C:\GIT\python_util\motivedns\subscriberIds.csv', encoding='ISO-8859-1', sep=';', dtype=str)
...
for index, row in df.iterrows():
    ...
    deviceGUID = consultar_motive(SUBSCRIBERID)
    if not deviceGUID:
        print(index, nrc, SUBSCRIBERID, 'Não existe deviceGUID')
        print('')
        continue

    processado = reboot_motive(deviceGUID)

    print(index, nrc, SUBSCRIBERID, deviceGUID, processado)
    print('')
    contador += 1